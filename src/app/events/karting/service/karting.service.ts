...
import {
  ...
  PostNewUserResponse,
} from '../interfaces/karting.interface';
...
export class KartingService {
  ...
  loadEvents(): Observable<Boolean> {
    return this._http
      .get<LoadEventsResponse>('http://localhost:3000/karting/getEventsList')
      .pipe(
        tap((res) => {
          if (res.result) {
  ...
  loadDataByEventId(event_id: number): Observable<Boolean> {
    return this._http
      .get<LoadDataByEventIdResponse>(
        `http://localhost:3000/karting/getEventResults/${event_id}`
      )
      .pipe(
        tap((res) => {
          if (res.result) {
  ...
  loadClassificationData(): Observable<Boolean> {
    return this._http
      .get<LoadClassificationData>(
        'http://localhost:3000/karting/karting-classification'
      )
      .pipe(
        tap((res) => {
          if (res.result) {
  ...
  registerUserForNextEvent(user_id: number): Observable<Boolean> {
    ...
    return this._http
      .post<PostNewUserResponse>(
        'http://localhost:3000/karting/registerUserForNextEvent',
        {
          user_id: user_id,
        },
        { headers }
  ...
  updateEventResult(
    ...
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
    return this._http
      .put<PostNewUserResponse>(
        'http://localhost:3000/karting/updateEventResults',
        {
          event_id: event_id,
          user_id: user_id,
          position: position,
   ...
   * - false si recibe un error
   */
  upsertEvent(object: EventResponse): Observable<Boolean> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
